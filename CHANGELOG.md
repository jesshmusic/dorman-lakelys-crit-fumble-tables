...
- **Wild magic surges on spell fumbles.** The "Wild Surge Jolt" result in tiers 2-4 became **Wild Magic Surge**, and tier 1 gained one at 99-100. When it comes up the module rolls on a wild magic table **behind the scenes** — the player is never asked to roll and the table posts no card of its own — and the surge is shown inside the fumble card itself, alongside the jolt damage the result already dealt. Wild magic is a flag rather than an effect type, so the result still applies its own effects too.
